...
from lib.hardware_config import MAKE_PIN, UP_PIN, DOWN_PIN, MISS_PIN
from lib.models import State_Machine, Game_Stats
...
async def timer_worker():
    ...
    while True:
            ...
            if utime.ticks_diff(now, last_tick) > 1000:
                ...
                if game.countdown >= 0:
                    if curr_tens != new_tens and curr_units != new_units:
                        ...
                        display.display_text(OLED, state_machine, display.process_timer_duration(game.countdown), 0, 0, 8)
        ...
        # 2. Handle Flashing (Time Over)
            ...
            if utime.ticks_diff(now, flash_checker) > 330:
                flash_checker = now
                if flash_off:
                    display.display_text(OLED, state_machine, display.process_timer_duration(game.countdown), 0, 0, 8)
                    flash_off = False
        ...
        if state_machine.profile_selection or state_machine.menu or state_machine.editing_value:
            if utime.ticks_diff(now, blink_checker) > 500:
                blink_checker = now
                blink_off = not blink_off
                
                if state_machine.profile_selection:
                    # Only blink after a short period of inactivity
                    if utime.ticks_diff(now, inactivity_check) > 500:
                        if blink_off: display.display_clear(OLED, "profile_selection")
                        else: await display.render_profile_selection(state_machine, game, OLED)
                
                elif state_machine.menu:
                    # Blink the cursor square
                    if blink_off: display.display_clear(OLED, "menu_selector")
                    else: display.display_shape(OLED, "rect", 8, 40, 8, 8, True)
