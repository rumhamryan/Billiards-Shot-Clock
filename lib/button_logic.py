...
async def handle_make(state_machine, game, hw_module):
    ...
    if state == State_Machine.PROFILE_SELECTION:
    ...
    elif state in [State_Machine.COUNTDOWN_IN_PROGRESS, State_Machine.COUNTDOWN_COMPLETE]:
        ...
        if sel == "Rack": game.rack_counter = game.temp_setting_value
        elif sel == "Mute": game.speaker_muted = game.temp_setting_value
        elif sel == "Inning": game.inning_counter = float(game.temp_setting_value)
...
async def handle_up(state_machine, game, hw_module):
    ...
    if state == State_Machine.PROFILE_SELECTION:
        game.profile_selection_index = (game.profile_selection_index - 1) % len(game.game_profiles)
